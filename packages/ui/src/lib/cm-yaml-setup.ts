...
export const cmEditorTheme = EditorView.theme({
  ...
  '.cm-panel button': { backgroundColor: 'var(--color-surface)', backgroundImage: 'none', border: '1px solid var(--color-border)', color: 'var(--color-fg)', borderRadius: '3px', cursor: 'pointer', padding: '1px 6px' },
  '.cm-panel button:hover': { backgroundColor: 'var(--color-surface-hover)' },
  '.cm-panel.cm-search label': { color: 'var(--color-fg)' },
  '.cm-panel.cm-search [name="close"]': { color: 'var(--color-fg)', backgroundColor: 'transparent', border: 'none' },
