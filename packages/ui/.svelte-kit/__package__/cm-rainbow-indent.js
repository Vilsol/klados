...
function buildBgGradient(level) {
    const stops = [];
...
function buildLineGradient(level) {
    const stops = [];
    for (let k = 0; k < level; k++) {
        const color = COLORS[k % COLORS.length];
        const pos = (k * INDENT_UNIT + 0.9).toFixed(1);
        stops.push(`transparent ${pos}ch`, `${color} ${pos}ch`, `${color} calc(${pos}ch + ${WIDTH_PX}px)`, `transparent calc(${pos}ch + ${WIDTH_PX}px)`);
    }
    return `linear-gradient(to right, ${stops.join(', ')})`;
}
// Pre-compute gradients for common indent depths
const gradientCache = new Map();
function getGradients(level) {
...
class RainbowIndentPlugin {
    ...
    build(view) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (level <= 0)
                continue;
            builder.add(lines[i].lineFrom, lines[i].lineFrom, Decoration.line({ class: 'cm-ril', attributes: { style: `--ril-bg: ${getGradients(level).lines}; --ril-fill: ${getGradients(level).bg}` } }));
        }
