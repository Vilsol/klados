...
const {
  mockInstallPlugin,
  mockListPlugins,
  mockSaveRegistryCredentials,
  mockAddInsecureRegistry,
  mockBrowsePluginFile,
} = vi.hoisted(() => ({
...
describe('PluginManagement — registry install', () => {
  ...
  it('normalises bare ref by prepending oci://', async () => {
    ...
    await waitFor(() =>
      expect(mockInstallPlugin).toHaveBeenCalledWith('oci://ghcr.io/foo/bar:v1'),
    )
  })
  ...
  it('submitCredentials calls SaveRegistryCredentials then retries InstallPlugin', async () => {
    ...
    await waitFor(() =>
      expect(mockSaveRegistryCredentials).toHaveBeenCalledWith('ghcr.io', 'myuser', 'mytoken'),
    )
  ...
  it('shows error message when credentials are rejected on retry', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Credentials rejected — verify and try again')).toBeTruthy(),
    )
