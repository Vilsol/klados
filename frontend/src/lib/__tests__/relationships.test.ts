...
import {getControllerRef, gvrToApiVersion, buildKindGVRMap, resolveGVR, type APIResource} from "$lib/utils/relationships";

describe("getControllerRef", () => {
  it("returns the controller:true ref", () => {
    const obj = {
      metadata: {
        ownerReferences: [{apiVersion: "apps/v1", kind: "ReplicaSet", name: "rs-abc", uid: "uid1", controller: true}],
      },
    };
  ...
  it("returns null when none has controller:true", () => {
    const obj = {
      metadata: {
        ownerReferences: [{apiVersion: "apps/v1", kind: "ReplicaSet", name: "rs", uid: "uid1", controller: false}],
      },
    };
