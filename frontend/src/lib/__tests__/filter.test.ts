...
type TestItem = {metadata: {name: string; namespace: string; labels: Record<string, string>; annotations: Record<string, string>}};

function makeItem(name: string, namespace: string, labels: Record<string, string> = {}, annotations: Record<string, string> = {}): TestItem {
  return {
...
describe("filterItems", () => {
  it("returns all items when no terms", () => {
    expect(filterItems(items as unknown as Record<string, unknown>[], [])).toHaveLength(4);
  });

  it("filters by bare text on name", () => {
    const terms: SearchTerm[] = [{type: "text", value: "nginx", negated: false}];
    const result = filterItems(items as unknown as Record<string, unknown>[], terms);
    expect(result).toHaveLength(2);
    expect((result as TestItem[]).map((r) => r.metadata.name)).toEqual(["nginx-proxy", "nginx-ingress"]);
  });
  ...
  it("filters by name qualifier", () => {
    ...
    const result = filterItems(items as unknown as Record<string, unknown>[], terms);
    expect(result).toHaveLength(1);
    expect((result[0] as TestItem).metadata.name).toBe("nginx-proxy");
  });
  ...
  it("negates text filter", () => {
    ...
    const result = filterItems(items as unknown as Record<string, unknown>[], terms);
    expect(result).toHaveLength(2);
    expect((result as TestItem[]).map((r) => r.metadata.name)).toEqual(["redis-master", "test-pod"]);
  });
  ...
  it("handles annotation with dots and slashes in key", () => {
    ...
    const result = filterItems(items as unknown as Record<string, unknown>[], terms);
    expect(result).toHaveLength(1);
    expect((result[0] as TestItem).metadata.name).toBe("test-pod");
  });
});
