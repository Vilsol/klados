<script lang="ts">
  ...
  onMount(async () => {
    container.addEventListener('focusin', () => shortcutStore.setMode('editor'))
    container.addEventListener('focusout', (e) => {
      if (!container.contains(e.relatedTarget as Node)) shortcutStore.setMode('normal')
    })
    ...
    if (ctxName && gvr && kind) {
      try {
        const s = await SchemaService.GetSchema(ctxName, gvr, kind)
  ...
  async function save() {
    ...
    try {
      ...
      const result = await ResourceService.UpdateResource(ctxName, gvr, namespace, parsed)
      if (result) {
        obj = result
        editing = false
        initEditor(makeYaml(result), true)
        notificationStore.push('Changes applied.', 'success')
      }
    } catch (e: any) {
      ...
      if (msg.includes('409') || msg.toLowerCase().includes('conflict')) {
        ...
        notificationStore.push(msg, 'error')
      }
    } finally {
      saving = false
    }
  ...
  async function handleConflict() {
    try {
      const latest = await ResourceService.GetResource(ctxName, gvr, namespace, name)
      if (latest) {
  ...
  function copyYaml() {
    navigator.clipboard.writeText(makeYaml(obj)).then(() => {
      notificationStore.push('Copied YAML to clipboard', 'success')
    })
  }
</script>
