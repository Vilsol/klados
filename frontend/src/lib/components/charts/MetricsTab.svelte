<script lang="ts">
  ...
  function getContainerSeries(container: string, metricName: string): TimeSeries[] {
    return getSeriesForMetric(metricName).filter(
      (s) => (s.labels['container'] ?? s.labels['pod']) === container
    )
  }
  ...
  <div class="flex flex-col gap-4 p-4 overflow-auto h-full">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        <TimeRangeSelector
          value={rangeMinutes}
          hasPrometheus={capability.hasPrometheus}
          onchange={(v) => (rangeMinutes = v)}
        />
        <button
      ...
      <span class="text-xs text-muted">
        {capability.hasPrometheus ? 'prometheus' : 'metrics-server (live only)'}
      </span>
    </div>
