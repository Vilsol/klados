...
<div class="flex flex-col h-full overflow-hidden">
  ...
  <div class="flex items-center border-b border-border bg-surface shrink-0 overflow-x-auto">
    {#each tabs as tab}
      <div class="flex items-center border-b-2 {activeTab === tab.id ? 'border-accent' : 'border-transparent'}">
        <button
          type="button"
          onclick={() => activeTab = tab.id}
          class="px-4 py-2 text-xs font-medium whitespace-nowrap transition-colors
            {activeTab === tab.id ? 'text-accent' : 'text-muted hover:text-fg hover:bg-surface-hover'}"
        >
          {tab.label}
        </button>
      </div>
