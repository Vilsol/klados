<script lang="ts">
  ...
  function getCurrentMetric(m: any): string {
    ...
    if (type === 'Resource') return found.resource?.current?.averageUtilization != null
...
<div class="p-4 space-y-6">
  ...
  {#if scaleTargetRef.name}
    <section>
      ...
      <div class="flex items-center gap-2 text-xs">
        <span class="px-1.5 py-0.5 rounded bg-surface border border-border text-muted font-medium">
          {scaleTargetRef.kind ?? ''}
        </span>
        <button
          onclick={() => push(scaleTargetURL())}
  ...
  <section>
    ...
    <div class="space-y-2">
      <div class="relative h-6 bg-surface border border-border rounded-full overflow-hidden">
        <div
          class="absolute left-0 top-0 h-full bg-accent/30 rounded-full transition-all"
          style="width: {gaugePercent()}%"
        ></div>
        {#if desiredReplicas !== currentReplicas}
          <div
            class="absolute top-0 h-full w-0.5 bg-yellow-400"
            style="left: {desiredPercent()}%"
          ></div>
        {/if}
      </div>
      <div class="flex justify-between text-xs text-muted">
        <span>min: {minReplicas}</span>
        <span class="font-semibold text-fg">current: {currentReplicas}{desiredReplicas !== currentReplicas ? ` (desired: ${desiredReplicas})` : ''}</span>
  ...
  {#if specMetrics.length > 0}
    <section>
      ...
      <DataTable
        columns={[{ label: 'Type' }, { label: 'Name' }, { label: 'Target' }, { label: 'Current' }]}
        items={specMetrics}
      >
        {#snippet row(m)}
