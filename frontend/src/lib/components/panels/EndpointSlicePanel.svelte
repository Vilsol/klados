...
<div class="flex flex-col gap-6 p-4 overflow-auto">
  <section>
    <SectionHeader> Address Type: <span class="ml-1 normal-case tracking-normal font-mono">{addressType}</span> </SectionHeader>
  </section>
        ...
        <tbody>
          {#each endpoints as ep}
            ...
            <tr class="border-t border-border">
              ...
              <td class="px-2 py-1.5"><StatusBadge status={cond.ready ?? false}>{cond.ready ? 'True' : 'False'}</StatusBadge></td>
              <td class="px-2 py-1.5"><StatusBadge status={cond.serving ?? false}>{cond.serving ? 'True' : 'False'}</StatusBadge></td>
              ...
              <td class="px-2 py-1.5">
                {#if ref?.kind === 'Pod'}
                  <a href="/c/{ctxName}/core.v1.pods/{ref.namespace ?? ns}/{ref.name}" class="text-accent hover:underline font-mono"
                    >{ref.name}</a
                  >
                {:else if ref}
