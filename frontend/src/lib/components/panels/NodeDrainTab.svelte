...
<div class="flex flex-col h-full overflow-hidden p-4 gap-3">
  <div class="flex items-center justify-between">
    ...
    {:else if terminalState !== 'active'}
      <span class="text-xs px-2 py-0.5 rounded font-medium {terminalState === 'complete' ? 'bg-green-500/20 text-green-400' : terminalState === 'error' ? 'bg-red-500/20 text-red-400' : 'bg-muted/30 text-muted'}">
